...
| [darkbloom-d-inference](case-studies/darkbloom-d-inference/) | 0 | First **edge-TEE** study (Apple Silicon, no app-level memory encryption). 2026-08 paid relaunch shipped a real remote code-identity anchor — an Apple-gated APNs challenge answered with a Secure-Enclave signature — replacing the self-reported `binaryHash` we flagged in June. Top gap **N1**: the proof isn't bound to the device it vouches for. Posture (MDM), key possession (bare SE key, no Apple cert), and code identity (a provider-supplied APNs token) are three legs pivoting on a **self-asserted serial**, and `/ws/provider` takes no auth — so one clean enrolled Mac can vouch for inference served anywhere. Also **N3**: the coordinator's TEE leg regressed from attested Intel TDX with a public image-digest history to a self-run GCE VM on **AMD SEV (not SEV-SNP)** with nothing published |
